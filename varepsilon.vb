...
' directory. Each line of file is expected to contain for space-separated
' fields:
'
'     <X> <Y> <Z> <EPSILON>
'
' which describe discrete mapping from coordinated to permettivity. The file
' *must* contain an exessive number of lines because each cell is filled by
' material which epsilon is the average of epsilons of all points which are
' located within that cell. Points read from file are additionally shifted by
' x0, y0, z0 offsets.
'
' Material permittivity are also quantized, the number of levels can be
' specified Nepsilon parameter.
'
